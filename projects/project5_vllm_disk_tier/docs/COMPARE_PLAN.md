...
**(⚠ 本节以下动机与判据写于第 0 步摸底之前,建立在"fs=buffered 读、走 page cache"的旧假设上——第 21 行的源码三问已证 fs 实为 O_DIRECT,热组照样从设备读满 4.3GB,page cache 轴不存在。保留原文是因为 E1 本身就是验证这个假设的实验,实测把它证伪了:热区均值打平、uring 赢在稳定性与冷区,结论见第 55 行后与第 137 行。读下面几段时按"待验证的假设"理解,不是结论。)** fs 是 buffered 读,只要数据还在 page cache 里,"读盘"就是读 RAM;uring 是 O_DIRECT,永远真读设备。所以第一个要控制的变量就是"盘上的数据在不在 page cache 里"。做法是 2×2 共四次 serve:fs/uring 两种 tier,各跑"热"(不压内存,空闲内存远大于工作集)和"冷"(ballast 把空闲内存压到小于工作集)两种条件,负载沿用现版的 prime→churn→revisit 串行。
...
**ballast 的作用被高估了(2026-07-14,元数据微基准的副产品,详见 BENCH_ANALYSIS §14)。** 本节两处措辞要修。其一,"冷/热"这个词在本文档里指的是**内核 page cache 的温度(= 宿主内存受压与否)**,不是 KV 块的访问频率——后者由 churn 构造,是所有组共有的前提而非变量。两个"冷"撞在一个词上,命名欠佳,读的时候要分清。其二更实质:**ballast 压根压不住元数据缓存。** 它的目标值(留 4 GiB 余量)是按 12.8 GB 的**数据**工作集定的,而 15574 个 inode 加 dentry 才几 MB——4 GiB 装得绰绰有余。微基准实测:ballast 档与一次性 drop3 档的衰减曲线一模一样(301→11 vs 287→11 us/桶),两者都在头 100 次读之后就把元数据缓存重建回来了。所以 E1 冷组里 fs 的元数据面之所以是冷的,**主因是 churn 持续写入几万个新文件冲刷缓存,不是 ballast 的内存压力**。这个修正扩大而非削弱了结论的适用面:E3/E4 根本没压舱,fs 照样系统性输,正因为冲刷源本来就不是 ballast。
...
(3) **fs 写带宽下滑:负结果,记成有边界的证据。** 26k 文件、90s 持续写、~340MB/s 下 store 吞吐无衰减,吸收率 81%(22.4G / 27.7G 上界),serve 日志 store 零失败。这排除了"文件数到 2.6 万就拖垮 store"的版本;但 E3 的条件(4 万文件、30+ 分钟、churn 与并发 load 交错)未复现,吸收率之谜(23% vs 81%)仍开放,两个候选(随文件数/时长衰减、静默丢块)都活着。不阻塞任何冻结结论(本轮两 tier 落盘对称);真要查,最便宜的设计是纯 churn 循环逐轮记"落盘增量 vs 文件数"曲线,排 E6 之后,可做可不做。

**uring_a "异常"结案:churn prefill 相位共振,不是 tier 问题。** 表面现象:uring_a mixed 457ms、劣化 +78% 且逐波恶化(389→524),b/c 却 ~315ms、逐波持平。设备层无辜——两波读量、r_await(~2ms)、aqu(8~10)全同。批次明细指认凶手:每波 16 条按 c=8 分两批,**批均值 ≈ 在飞 churn 的剩余 prefill 时间 + ~250ms 本征**(wave1 批2:churn prefill 剩 52ms→批 292ms;wave2 批2:剩 490ms→批 490ms)。为什么 a 两波全撞、b/c 两波全不撞:mixed churn 是闭环流、周期 1.24~1.25s,波间隔 20s ≈ 16.0 个周期,**相位在 serve 内被冻住**——抽中坏相位就两波全坏。六组 24 个批次的相位检查全部吻合(b/c 的批到达时 churn 要么 prefill 只剩 12~67ms、要么已在 decode 中)。这是负载设计里埋的共振(波间隔 ≈ churn 周期整数倍);合并 a/b/c 恰好是对的口径,后续设计要么把波间隔与 churn 周期取互质,要么继续靠多 serve 合并摊掉相位运气。
...
**项目实验部分到此收口。** E1/E3/E4/E5 冻结,E6 按负结果冻结,E7 以论证代替实验。剩下的都是不阻塞结论的可选悬案:fs 吸收率之谜(23% vs 81%,§77/§98,查法是纯 churn 循环记"落盘增量 vs 文件数"曲线)、E4 读路径劣化的两条候选机制不可分辨(§109,要分开需再设计 rename → O_DIRECT 原地写的对照)、churn TTFT 系统性 +10ms 的无主悬案(§55)。三条都要重新上机,做与不做都不改矩阵。