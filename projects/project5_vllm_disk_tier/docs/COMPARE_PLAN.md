...
**设计:纯 churn 循环,把 load 从环境里整个拿掉。** 两 tier × a/b/c 交替 serve,phase 只有 prime + churn × N 轮(无 revisit、无波),churn 总量按 E4 的上界公式压在容量内(零逐出、写量对称)。逐轮记三样:(a) 落盘增量 vs 文件数/时长曲线——直判吸收率之谜:纯 churn 若吸收 ≈8 成,坐实"load 波打断下沉"候选;若掉回 2 成档,指向文件数/时长衰减。(b) churn TTFT 相对无 offload 基线(A 组)的差——store 的当期税;10ms 信号小,靠多轮合并压方差;若 +10ms 在纯写环境复现,pidstat -t 加 1s iostat 就地定位通路,若不复现,说明它依赖与 load 交错,指向读写互斥点。(c) 尾延迟节奏与元数据事务形态(继承原 E2 的观测遗产:周期性 spike 还是全程平)。指标结构上,store 的**成本**与**产出**分开记:成本 = TTFT 税,产出 = 吸收率/落盘完整性——store 慢的真实代价是块没落上盘、未来 load MISS 转重算,以命中率的形式付账,只看 TTFT 会漏掉重的那半。工具缺口一件:uring 侧目前没有落盘字节的引擎账本(fs 有文件账),需加计数器,否则吸收率对比只有一半。
