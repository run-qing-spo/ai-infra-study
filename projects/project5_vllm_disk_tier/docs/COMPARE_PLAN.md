...
**干净对照:唯一块总量压到 20G 以内(2026-07-14 定案)。** E3 暴露的 2.9× 写放大(uring 101G vs fs 35G)必须在 E4 之前解决,否则"写流干扰读路径"这个测量对象会被"uring 自己的逐出重写"污染——两边根本不是同一场比赛。根源已定位在容量:`submit_store` 本来就过滤盘上已驻留/在途的 key(manager.py:222),所以重写只可能来自 `disk_bytes_to_use=20G` < 盘上工作集 ~35G 逼出的 LRU 逐出。两个选项里选**不动环境**的那个:把 E4 的盘上唯一块总量设计到 20G 以内,使 uring 的容量上限失效(全程零逐出),两边都是"写一次、留着",写量对称。代价是 mixed 阶段的时长被写速率硬顶死,不能靠加波数拿样本。

预算按 E1/E3 的落盘实测标定,不靠猜:块大小 0.875MiB(16 token × 56KB);prime 的 16 条前缀 = 8000 块 = **7.0G**(revisit 复用同一批块,不产新块);churn 的唯一块产出速率**不是常数**——E1 单轮实测 ~276MiB/条(冷缓存),E3 十二轮均值 98.6MiB/条,两点反解出稳态 ≈ 82MiB/条 ≈ 71MB/s(与 gil 那节记的"churn 期 ~85MB/s store 流量"吻合)。于是每个 serve 的账:prime 7.0G + churn 首轮(24 条,冷)6.6G + mixed 两波(churn 流约 58s)4.1G ≈ **17.7G**,距 20G 留 11% 余量。作废条件写进脚本:fs 组收尾统计的落盘量就是本负载的唯一块总量(fs 按内容哈希天然去重、无上限,且两组跑同一批 prompt),它一旦 > 20480MiB,uring 组必然逐出重写 → 标 `WARN(capacity)`,该轮对照作废。

顺带这一步把写放大的归因变成一个**可证伪的实验**:唯一块 ≤ 20G 之后,uring 的全程 md0 写量应当降到 ≈ fs 的唯一块字节数。若它仍然是 2~3 倍,说明"容量逼出 LRU 逐出"的归因是错的,盘上有真 bug(key 不稳定、或有别的路径在重写),得回头查 manager。
...
2026-07-14:E4 的工具与实验矩阵已就绪,Mac 侧无 GPU 的活干完了。`--mixed` 和 ITL 探针实装 + 离线冒烟通过;`bench/run_e2e_overnight.sh e4` six 组 a/b/c 交替(fs/uring × 3),phase 为 `prime,churn,revisit@8,mixed@8`(同 serve 自带安静盘基线),盘上唯一块按预算压到 ~17.7G < uring 的 20G 上限以求干净对照,越界自动标 `WARN(capacity)` 作废。接下来只剩上 GPU 机执行:跑 e4 → E5 从 E4 的 ITL 探针出裁决(证据源一已为负,详见 E5 节)→ E6 用 Mooncake trace 收尾。全程沿用 overnight 脚本的无人值守骨架。