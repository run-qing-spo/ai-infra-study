...
**为什么打不到盘。** 三条旁证同向。`External prefix cache hit rate` 全程只有 0~3.3%,而 GPU 侧 `Prefix cache hit rate` 有 30~45%——复用被 GPU prefix cache 和 4G CPU tier 截留在上层了,块确实被 store 到了盘(27.7GB),但没有人再从盘上把它读回来。那仅有的 0.46GB 读全部发生在窗口后 2/3(t+44s 与 t+62s,窗口总长 66s),是"刚要开始激活、回放就结束了"的形状。第三条最能说明问题:c=16 的设备平均请求尺寸 fs 446.8KB ≈ uring 441.6KB,两者几乎相同——这些文件是几十秒前刚写的,dentry 全在缓存里,**E3/E4 里 fs 的那个劣化源(冷 dentry 的 8×4KiB 元数据读)在这个窗口里根本不存在**。

**负结果的信息:逻辑复用率 ≠ 物理盘读。** dry-run 把"block 被复用"等同于"要从盘读",实测证明这个等号不成立——中间隔着一整层 GPU/CPU tier 的截留。`load_bytes` 随并发单调爬升(0 → 0.02 → 0.72GB)画出了激活曲线:真正打开磁盘 tier 的开关是**逐出压力**,不是复用率。这正是 E1/E3/E4 必须用 churn 当压力泵的原因(churn 是人工的逐出压力源),也把 E7 那条"高局部性 block 被上层 tier 留住、沉不到盘"的纯论证变成了实测旁证。要在单节点上让真实 trace 激活磁盘 tier,需要的是长时程 / 大工作集下的持续逐出压力,而这恰恰撞上 COMPARE_PLAN §127 已经承认的尺度错配:整条 trace 的累计 KV 是 TB 级,单卡 4090 + 36G 可用盘只装得下 150 条的窗口,而 150 条还不足以攒出逐出压力。**结论:磁盘 tier 的价值区间由逐出压力定义,而单节点尺度的连续 trace 窗口够不到它——这条边界是 E6 的真实产出,推理链在此如实断开,不假装闭合。**
