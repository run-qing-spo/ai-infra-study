...
## 14. 元数据面:把 E1/E3 的归因假说做成可证伪实验

E1 冷组和 E3 全档的 fs 劣化,一直挂着同一个**未验证的归因**:"每 block
一个文件 → 一万五千条 dentry/inode → 元数据缓存一失效,每次 load 的
`open()` 就要去盘上取"。e2e 证不了它,原因有三:ballast 同时压 page
cache 和 slab(混杂变量),TTFT 把 open 的成本埋在 prefill 里(看不见),
而且没有任何一组能单独关掉"文件布局"这一个自变量。

于是把它剥成纯 IO 微基准(`bench/meta_probe.cpp` + `run_meta_probe.sh`),
不要 GPU、不要 vLLM、不要模型 —— 它验的是 `open()` 路径,一台 Linux
加一块盘就够。宿主:ext4 on LVM(dm-0 over nvme0n1),14 GiB RAM,
15000 个块 × 0.875 MiB(对齐 E1 实测的 15574 个 kv_fs 文件),每档随机
读 10000 次。原始数据 `results_meta_20260714_0949/`。

### 设计:让假说可证伪的那把刀

布局三组,读路径完全对齐(都是 O_DIRECT、同块大小、同随机块序列):
`files`(N 个小文件,hash 三层子目录,每次读 open 一个不同文件)、`slab`
(单文件复用 fd)、**`slab-reopen`(单文件,但每次读都重开一遍)**。

第三组是刀刃。它和 `files` 的 **open 次数一模一样**,唯一差别是 dentry
有 15000 条还是 1 条。冷条件下若 `files` 涨而 `slab-reopen` 不涨,贵的
就不是 `open()` 这个 syscall 本身,而是"N 条 dentry 装不下/被回收";
两者一起涨则假说当场被证伪,E1/E3 的归因得推倒重来。

缓存条件四档,靠 `drop_caches` 做因子分离 —— 这是 ballast 给不了的:
`hot`(全热)、`drop2`(**只**丢 dentry/inode slab,重建它们要读的目录
块/inode 块还在 page cache → 有 CPU 重建成本、不读盘)、`drop3`(slab +
page cache 全丢 → 重建必须真读盘)、`ballast`(持续内存压力,和 e2e 对齐)。
`drop2` 与 `drop3` 之差 = 纯 CPU 重建;`drop3` 与 `drop2` 之差 = 盘上元
数据 IO。**两笔账第一次分开量。**

一个方法论坑值得记:冷条件会**自我衰减**。元数据体积很小(15000 个 inode
才几 MB),一次性 drop 之后头 ~100 次读就把它读回 page cache 了,后面全
是热态 —— 首版实测均值被稀释 85%(衰减曲线 287→11 us/桶),而且 iostat
的 1 秒粒度根本抓不到那 0.07 秒的冷窗口。解法是 `--redrop-every 100`:
每 100 次读重清一遍,全程冷。改完衰减归零(各档前后半差 ±1%),均值才是
稳态值。**这也更贴 e2e** —— 那里 churn 一直在写新文件,元数据缓存本来
就在被持续冲刷。
...
`files` 43×,`slab-reopen` 在同样的 open 次数下纹丝不动(+0.4us)。
**假说成立,反面没出现。** 分账:纯 CPU 路径重建 8.7us,盘上元数据 IO
141.9us —— **IO 是 CPU 的 16 倍**,"回收会占 CPU"确实存在但不是主要矛盾。

iostat 给出机制的直接观测(dm-0,同一块盘同样的数据量,唯一变量是布局):
...
**每次 `open` 要付约 8 次额外的设备读。** 9.2s × 9757 r/s ≈ 89764 次设备
读,减去 10000 次数据读,79764 ÷ 10000 = 8.0。这个数对得上 ext4 的路径
结构(三层目录 + 文件,每级要读目录块和 inode 块)。平均尺寸也验算得通:
(10000 × 896 KiB + 79764 × 4 KiB) ÷ 89764 ≈ 103 KiB,实测 88.5 KiB。
**代价是 29% 的数据吞吐:1340 → 953 MiB/s** —— 4 KiB 小读吃掉了设备的
IOPS 和队列深度预算,大块数据读只能排队。

### 与 E3 闭环

E3 在真实 e2e 上观测到的是:设备请求尺寸 fs 135~250 KB vs uring
342~500 KB,fs 的 aqu-sz 在 c=16 掉到 4~6。当时归因写的是"fs 的每文件
元数据路径既偷走设备队列深度、又碎掉 IO 尺寸"—— 那是从现象反推的假说。
本节把它变成了直接观测:同盘、同数据量、唯一变量是文件布局,请求尺寸
773 → 88.5 KiB、请求数 7.4×。**两个实验的证据链闭合,E3 的归因从假说
升级为已验证。**(e2e 的 135~250 KB 没碎到 88.5 KiB,是因为那里元数据
缓存部分命中;方向与量级都自洽。)

并发轴不需要在微基准里重扫:请求数、请求尺寸、吞吐损失都是**设备层**
的量,与用几条线程去 open 无关 —— 8 次元数据设备读一次都少不了,队列
预算照样被吃掉。并发下会怎样 E3 已在 e2e 上扫完(c=1→16 全档 fs 输,
差距 8%→28% 放大),微基准再扫一遍是重复劳动。

### 归因修正:ballast 不是 E1 冷组的主因

首版(未开重 drop)的数据反过来打了 ballast 一巴掌:`ballast` 与 `drop3`
的衰减曲线一模一样(301→11 vs 287→11 us/桶),**两者都衰减了 85%**。
也就是说 **ballast 那 4 GiB 的余量根本压不住元数据缓存的重建** ——
15000 个 inode 才几 MB,4 GiB 装得绰绰有余。ballast 只压住了数据缓存
(它的目标值是按 12.8 GB 的**数据**工作集定的),元数据从来不在它的射程内。

所以 E1 冷组里 fs 的元数据面之所以是冷的,**主因不是 ballast 的内存压力,
而是 churn 阶段持续写入几万个新文件、不断冲刷元数据缓存**。这个修正不
削弱结论,反而扩大了它的适用面:**E3 和 E4 根本没压舱**,fs 照样系统性
输 —— 那里的元数据缺失本就来自"文件数太多 + 持续写入冲刷",而这正是
`drop3` 档模拟的状态。**本节解释的是 E3/E4 的主战场,不只是 E1 的边角。**
E1 的归因表述相应改成"内存压力叠加 churn 冲刷",不能只记在 ballast 头上。

一个读数陷阱记录在案:所有 `ballast` 档的墙钟都涨到 24~25s、吞吐掉到
343~368 MiB/s,**包括 slab 和 slab-reopen**。这是内存压力带来的系统级
减速(页回收、分配路径变慢),对三种布局**对称**,不影响 open 延迟的横向
对比,但那几个吞吐数字不可跨档比较。

### 面试版一句话

"fs tier 输在哪,我没停在'大概是元数据开销'。我把它做成了一个可证伪的
微基准:同一块盘、同样的数据量、O_DIRECT 读同样的块序列,唯一的变量是
文件布局 —— 而且我加了一组'单文件但每次重开',让它和多文件组的 open
次数完全相同,只差 dentry 是一条还是一万五。缓存被回收后,多文件组的
open 从 3.6us 涨到 154us,单文件重开组纹丝不动 —— 贵的不是 open 这个
syscall,是那一万五千条 dentry。iostat 把机制照出来了:每次 open 要付
8 次额外的设备读,设备平均请求尺寸从 773 KiB 崩到 88.5 KiB,数据吞吐掉
29%。这正好解释了 e2e 里 fs 的请求尺寸只有 uring 的一半、队列深度喂不满
设备。顺带这个实验还纠正了我自己一个错:我原以为是压舱把元数据挤掉的,
数据显示压舱压根压不住它 —— 真正的冲刷来自 churn 持续写入的几万个新文件,
所以这条归因在不压舱的 E3/E4 上同样成立。"
