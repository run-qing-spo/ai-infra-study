...
- `_split_node` 会刷新 `child.last_access_time`，FULL 的驱逐序因此受 split 影响 —— 轮次 04 验证是否与 `evict` 的取序一致。
- `full_host_duplicates` 的注销是懒的，消费侧必须实时复检 —— 轮次 04 / 11 沿这条线确认复检是否覆盖 `load_back_pending_id`。
