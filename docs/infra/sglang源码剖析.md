...
> 本文梳理 SGLang 0.5.18 中 KV Cache 的核心数据结构与生命周期：GPU/Host/L3 三级存储如何组织、Radix Tree 如何做前缀复用、Prefill/Decode 如何分配与释放 slot，以及故障发生时的边界处理。共 16 节，可按目录跳读，也可以从头顺序阅读。
...
三者串联起来，一个 token 是这样找到自己的 K/V 的：

```mermaid
flowchart LR
    A["请求中的第 i 个 token"] --> B["req_to_token[req_id, i]"]
    B --> C["GPU slot_id"]
    C --> D["每层 KV Pool[layer][slot_id]"]
    D --> E["真实 K/V"]
```

Radix Tree 保存的是 token → slot 索引关系，**不**保存 K/V 张量本身。
...
1. 构造 `RadixKey(tokens, extra_key, cache_salt)`
2. 从根节点逐段匹配
3. 必要时执行 `split_node`
4. 拼接所有命中节点的 `value`
5. 返回 `prefix_indices` 和 `last_node`
...
- `page_size > 1` 时，`slot_id = page_id × page_size + offset`，`page_id` 由 allocator 分配。

页 ID 来自预先建立的空闲页集合，会循环复用，并非持续自增。上层仍按 token slot 使用；页的归属、对齐和释放由 allocator 管理。
...
1. 完整输入
2. Radix 命中，得到 `prefix_len`
3. `extend_len = input_len - prefix_len`
4. 为 `extend_len` 分配 slots
5. 写入 `req_to_token`
6. 模型逐层计算 K/V
7. 写入对应层的 GPU KV Pool
...
长 Prompt 被拆成多个 chunk，依次进入 Prefill：

```mermaid
flowchart LR
    P[Prompt] --> C1[chunk1]
    C1 -->|Prefill| C2[chunk2]
    C2 -->|Prefill| C3[chunk3]
    C3 -->|Prefill| Done[完成]
```
...
1. Decode 提前分配目标 slots
2. 报告本地已有 `prefix_len`
3. Prefill 只发送缺少的 KV
4. Decode 各 rank 轮询传输状态
5. TP / CP / PP 达成一致
6. 请求进入 Decode batch
